...
Both arguments `source` and `target` are represented using the format: `<organizationName>/<apiName>/<apiVersion>`. When the "policyId" argument is provided, only the policy with that matching ID is copied. +
For example:
...
The <assetIdentifier> argument must be formatted as: <groupId>/<assetId>/<version>. +
...
Arguments "source" and "target" must be formatted as: <groupId>/<assetId>/<version>.
...
Argument "assetIdentifier" must be formatted as: <groupId>/<assetId>/<version>. +
This command does not take any options, except for the default ones: `--help`, `-f`/`--fields` and `-o`/`--output`.
